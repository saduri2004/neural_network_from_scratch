...
class NeuralNetwork(ABC):
    ...
    def backward(self, target: np.ndarray, out: np.ndarray) -> float:
        ...
        loss_forward = self.loss.forward(target, out)
        loss_backward = self.loss.backward(target, out)
        backprop_layers = reversed(self.layers)
        for l in backprop_layers:
            loss_backward = l.backward(loss_backward)
        return loss_forward
