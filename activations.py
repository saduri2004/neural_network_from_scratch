...
class SoftMax(Activation):
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        for i, value in enumerate(p):
            J = np.zeros((len(value), len(value)))
            for row in range(len(value)):
                for col in range(len(value)):
                    if row == col:
                        # Diagonal elements where i = j
                        J[row, col] = value[row] * (1 - value[col])
                    else:
                        # Off-diagonal elements where i != j
                        J[row, col] = -value[row] * value[col]
            gradient = np.dot(dY[i], J)
            backward.append(gradient)
